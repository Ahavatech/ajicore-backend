/**
 * Material Controller
 * Handles HTTP logic for inventory/material management.
 */
const materialService = require('./material.service');

async function getAllMaterials(req, res, next) {
  try {
    const { business_id, low_stock } = req.query;
    const materials = await materialService.getMaterials({ business_id, lowStock: low_stock === 'true' });
    res.json(materials);
  } catch (err) {
    next(err);
  }
...
module.exports = { getAllMaterials, getMaterialById, createMaterial, updateMaterial, deductMaterials };